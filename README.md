...
현대 비즈니스에서 SNS 광고는 필수적이지만, 전문 지식이 없는 소상공인에게는 기획, 디자인, 배포, 성과 관리가 큰 부담으로 작용합니다. 본 프로젝트는 **상품명과 상품 이미지**만 입력하면, AI가 자동으로 광고 콘텐츠를 생성하고 SNS에 배포하며, 성과를 분석하여 저조한 광고를 자동으로 교체하는 **완전 자동화 시스템**입니다.
...
본 시스템은 크게 **광고 콘텐츠 생성 모듈**과 **자동화 모듈**로 구성되어 있으며, 웹 기반의 통합 시스템으로 구현되었습니다.
...
| 구분             | 기술 (Technology)                         | 설명                                  |
| :--------------- | :---------------------------------------- | :------------------------------------ |
| **Frontend**     | React                                     | 사용자 입력 및 결과 확인 UI           |
| **Backend**      | Java, Spring Boot                         | API 서버 및 비즈니스 로직 처리        |
| **AI Models**    | Qwen2.5-VL (LoRA), ChatGPT-4o, Gemini 2.5 | 레이아웃, 텍스트, 배경 이미지 생성    |
| **Automation**   | Apache Airflow, Python                    | 광고 운영 프로세스 자동화 및 스케줄링 |
| **Database**     | MySQL, AWS RDS                            | 광고 데이터 및 성과 지표 저장         |
| **External API** | Meta Graph API                            | Facebook 광고 배포 및 인사이트 수집   |

---

## 📊 성능 평가 (Evaluation)

LoRA 파이프튜닝을 적용한 `Ours(LoRA)` 모델은 기존 `Qwen2.5-VL` 대비 다음과 같은 성능 향상을 보였습니다.

| Metric                | Qwen2.5-VL | Ours (LoRA) | 향상 효과                                   |
| :-------------------- | :--------: | :---------: | :------------------------------------------ |
| **Ove (Overlap)**     |   0.0649   | **0.0253**  | 요소 간 겹침 현상 대폭 감소 (낮을수록 좋음) |
| **Rea (Readability)** |   0.6124   | **0.9767**  | 가독성 크게 향상                            |
| **Und (Underlay)**    |   0.9442   | **0.9836**  | 텍스트 배경 처리 개선                       |
