import time
print("--- [DEBUG] 1. 스크립트 시작 (Imports 전) ---", flush=True)
...
print("--- [DEBUG] 2. Imports 완료 ---", flush=True)

# ==========================================
# 1. Global 설정 및 모델 로드 (서버 시작 시 1회 실행)
# ==========================================
print("--- [DEBUG] 3. Global 설정 및 모델 로딩 시작 ---", flush=True)
...
# 모델 & 프로세서 로드 (전역 변수)
try:
    print(f"--- [DEBUG] 모델 로딩 중... (ID: {MODEL_ID}, Device: {DEVICE}) ---", flush=True)
    ...
    print("--- [DEBUG] 프로세서 로딩 중... ---", flush=True)
    processor = AutoProcessor.from_pretrained(MODEL_ID)
    print("--- [DEBUG] 모델 및 프로세서 로드 완료! ---", flush=True)

except Exception as e:
    print(f"--- [FATAL ERROR] 모델 로딩 실패: {e} ---", flush=True)
    # 여기서 에러를 발생시켜야 RunPod이 재시작을 시도하거나 로그를 남김
    raise e

# ==========================================
# 2. Prompts & Schemas (상수)
# ==========================================
SYSTEM = (
...
def propose_fallback_text_visual(energy, subject_bbox, rules, hint=None, seed=None):
    ...
    h = min(h, 0.22)
    b = grid_search_low_energy(energy, win_w=w, win_h=h, margin=rules["min_margin"], avoid=[subject_bbox], stride=0.02, seed=seed)
...
def propose_fallback_logo_visual(energy, subject_bbox, text_bbox, rules, hint=None, seed=None):
    ...
    w = min(w, 1 - 2*rules["min_margin"])
    h = min(h, 0.15)
    b = grid_search_low_energy(energy, win_w=w, win_h=h, margin=rules["min_margin"], avoid=[subject_bbox, text_bbox] if text_bbox else [subject_bbox], stride=0.02, seed=(None if seed is None else seed+7))
...
def run_vlm_gen(image_path, product_name, cond, max_new_tokens, top_p, temperature):
    # 전역변수 model, processor 사용
    messages = [
...
def run_vlm_bg(image_path, product_name, context_str, palette, max_new_tokens, top_p, temperature):
    messages = [
...
def download_image(image_input):
    """URL 또는 Base64 문자열을 받아서 로컬 파일 경로로 반환"""
    ...
    # 임시 파일로 저장 (RunPod 작업 종료 후 삭제됨)
    temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
    image.save(temp_file.name)
    return temp_file.name

# ==========================================
# 5. Handler Logic (Main Entry Point)
# ==========================================
def handler(job):
    print(f"--- [DEBUG] Job 요청 수신: {job['id']} ---", flush=True)
    job_input = job["input"]
    
    # 1. 입력값 파싱 (Default 값 설정)
    image_data = job_input.get("image")  # 필수
    product_name = job_input.get("product_name", None)
    
    # 설정 옵션들
    cond = job_input.get("cond", {}) # 조건 JSON (규칙 등)
    max_new_tokens = job_input.get("max_new_tokens", 900)
    temperature = job_input.get("temperature", 0.7)
    top_p = job_input.get("top_p", 0.9)
    bg_min_chars = job_input.get("bg_min_chars", 900)
    
    # 플래그들
    bg_prompt_flag = job_input.get("bg_prompt", True) # 기본적으로 배경 생성
    no_fallback = job_input.get("no_fallback", False)
    no_rules = job_input.get("no_rules", False)
    relax_if_all_dropped = job_input.get("relax_if_all_dropped", True)
    fallback_strategy = job_input.get("fallback_strategy", "visual")
    ...
    try:
        ...
        # 4. VLM 실행 (Pass 1)
        print("--- [DEBUG] 1차 VLM 실행 ---", flush=True)
        parsed = run_vlm_gen(
        ...
        # 5. Visual Analysis (Sobel Energy)
        ...
        if no_rules:
            ...
            texts = enforce_text_rules(texts, subject_bbox, text_rules, debug)
            texts = nms(texts, iou_thr=0.3)
            texts = pick_single_text(texts, subject_bbox)
            
            logos = [g for g in graphics if g.get("type")=="logo"]
            logos = enforce_logo_rules(logos, subject_bbox, texts[0]["bbox"] if texts else None, logo_rules, debug)
        ...
        # 9. 배경 프롬프트 생성 (Pass 2)
        need_bg = (bg_prompt_flag or not (parsed.get("background", {}).get("prompt")))
        if need_bg:
            print("--- [DEBUG] 2차 배경 프롬프트 생성 중 ---", flush=True)
            palette = extract_palette_hex(image_path, k=5)
            context_str = summarize_layout_for_bg(parsed)
            bg_plan = run_vlm_bg(image_path, product_name, context_str, palette, max_new_tokens, top_p, temperature)
        ...
        print("--- [DEBUG] 작업 완료, 결과 반환 ---", flush=True)
        return parsed

    except Exception as e:
        print(f"--- [ERROR] 핸들러 실행 중 에러: {e} ---", flush=True)
...
# ==========================================
# 6. RunPod Start (Entry Point)
# ==========================================
# ★★★ 여기가 가장 중요합니다. 이 부분이 없으면 worker exited with exit code 0 에러가 납니다. ★★★
if __name__ == "__main__":
    print("--- [DEBUG] RunPod Serverless 리스너 시작... ---", flush=True)
